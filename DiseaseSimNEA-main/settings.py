...
# Function to set up input for particles and vaccine options
def particlesetup(screen):
  ...
  tick_img = pygame.transform.scale(
    tick_img, (tick_box_rect.width, tick_box_rect.height))  # Resize tick image
  ticked = False  # Flag to indicate if tick is displayed

  # Prepare initial display on the input screen
  ...
  text_vaccinations_surface = font.render(text_vaccinations_disabled, True,
                                          color)
  ...
  while True:
    for event in pygame.event.get():
      if event.type == pygame.QUIT:
        ...
        # Update the screen after text input and display tick image
        input_screen.fill((255, 255, 255))
        input_screen.blit(base_surface_particles, (50, 50))
        ...
        if ticked:
          input_screen.blit(
            tick_img, tick_box_rect.topleft)  # Draw tick image inside the box
          text_vaccinations_surface = font.render(text_vaccinations_enabled,
                                                  True, color)
        else:
          text_vaccinations_surface = font.render(text_vaccinations_disabled,
                                                  True, color)

        input_screen.blit(text_vaccinations_surface,
                          text_vaccinations_rect.topleft)
        pygame.display.flip()
